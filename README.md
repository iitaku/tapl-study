...
5.2「ラムダ計算でのプログラミング」ではChurch数の定義とその計算体系についてまとめられていました。

この体系は直感とは反する動きをする事があります。
たとえば、Churchブール値は真偽値を表し、Church数は整数値を表していました。
...
これをふまえると、簡約基(tru s (c1 s z)をc0 s zへと簡約していいものか甚だ疑問ですが、
...
`λs. λz. tru s (c0 s z)`で評価が止まったとしても、
これをrealnat等で評価すれば、0と評価される。
これは、いわゆる振る舞い等価ということではないか？
...
であり、仮定より`|FV(t1)∪FV(t2)| ≤ size(t1) + size(t2) + 1`となる。
定義より、`size(t1) + size(t2) + 1= size(t1 t2)`であるから、
...
E-APP1とE-APP2は3章でもやったように、評価戦略を決定する合同規則である。
